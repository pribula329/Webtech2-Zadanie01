...
if(isset($_POST["submit"])) {
    ...
    if($check !== false) {
        echo '<h2 class="upload"> Subor je korektný' .$check["mime"].' !</h2><br>';
        $uploadOk = 1;
    } else {
        echo '<h2 class="upload"> Subor nie je korektný !</h2><br>';
        $uploadOk = 0;
    }
}
...
if (file_exists('files/'.$_POST['nazov'].'.'.$FileType)) {
    echo '<h2 class="upload"> Subor s danym nazvom a priponou existuje!</h2><br>';

    $NazovNaUlozenie = $_POST['nazov'].'.'.time().'.'.$FileType;
    echo '<h2 class="upload"> Pre ulozenie súboru sme upravili názov na:</h2><br>'
            . '<h2 class="upload">'.$NazovNaUlozenie.'</h2>';
    $uploadOk = 1;
}
